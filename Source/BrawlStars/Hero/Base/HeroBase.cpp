#include "HeroBase.h"
#include "BrawlStars/Components/HealthComponent.h"
#include "BrawlStars/Components/SkillLockComponent.h"
#include "HeroType/AimingFlat.h"
#include "HeroType/AimingLaunch.h"
...
#include "BrawlStars/PlayerState/GameBase.h"
...
void AHeroBase::OnConstruction(const FTransform& Transform)
{
	...
	// 英雄初始化
	{
		...
		UDataTable* DataTable = LoadObject<UDataTable>(nullptr, TEXT("/Script/Engine.DataTable'/Game/Hero/Base/Data/DT_Hero.DT_Hero'"));
		RowNames.Empty();
	...
	// 技能蒙太奇
	{
		UDataTable* DataTable = LoadObject<UDataTable>(nullptr, TEXT("/Script/Engine.DataTable'/Game/Hero/Base/Data/DT_SkillMontage.DT_SkillMontage'"));
		RowNames.Empty();
		UDataTableFunctionLibrary::GetDataTableRowNames(DataTable, RowNames);
		for (auto& Elem : RowNames)
		{
	...
	// 英雄台词
	{
		UDataTable* DataTable = LoadObject<UDataTable>(nullptr, TEXT("/Script/Engine.DataTable'/Game/Hero/Base/Data/DT_HeroSpeakLine.DT_HeroSpeakLine'"));
		RowNames.Empty();
		UDataTableFunctionLibrary::GetDataTableRowNames(DataTable, RowNames);
		for (auto& Elem : RowNames)
		{
...
void AHeroBase::SpawnWeapon()
{
	if (WeaponType)
	{
		...
		if (World)
		{
			...
			Weapon = World->SpawnActor<AWeaponBase>(WeaponType, FVector::ZeroVector, FRotator::ZeroRotator, SpawnParams);
